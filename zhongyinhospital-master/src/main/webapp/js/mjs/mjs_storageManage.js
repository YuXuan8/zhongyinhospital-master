...
$(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('.drugTypeSelect').html(optionHtml).trigger("chosen:updated").chosen({
                ...
                width: '125px',
                max_shown_results: 10, //下拉框最大显示选项数量
    ...
    $.ajax({
        ...
        type: "post",
        dataType: "json",
        data:{"drug":"efficacy_classification"},
        success: function (data) {
            var optionHtml = '<option value=""></option>';
            $.each(data, function (i) {
                optionHtml += '<option value="' + data[i] + '" >' + data[i] + '</option>';
            ...
            $('.drugSelect').html(optionHtml).trigger("chosen:updated").chosen({
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $("#drugInfo_storageQuantity").val(data.drugCount)
                    }
                })
            });
        }
...
$('#add_StorageQuantity,#wholesalePrice,#price,#storageQuantity').keypress(function (e) {
    // 获取按键的ASCII码
    var keyCode = e.which ? e.which : e.keyCode;

    // 如果按键的ASCII码不是数字键（0-9），则阻止默认行为
